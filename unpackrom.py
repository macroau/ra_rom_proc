...
for i in trans_list:
    ...
    print()
    print('##### 即将传输 %s 分类的ROM包 #####' % MACHINE_NAME)
    
    ROM_PACK = MACHINE_NAME+'.tar.gz'
    THUMB_PACK = MACHINE_NAME+'.tar.gz'

    print(" *** 正在传输playlist文件，请稍后 ...")
    cmd('adb shell "mkdir -p %s"' % ADB_PLAYLIST_DIR)
    cmd('adb push "%s" %s/' % (LPL_DIR+os.sep+PLAYLIST_FILE, ADB_PLAYLIST_DIR))

    print(" *** %s ROM包大小 %d MB." % (MACHINE_NAME, int(os.path.getsize(DEST_ROM_DIR+os.sep+ROM_PACK)/1000/1000)))
    print(" *** 正在传输ROM压缩包，请稍后 ...")
    cmd('adb push "%s" %s/' % (DEST_ROM_DIR+os.sep+ROM_PACK, TF_ROOT))
    cmd('adb shell mkdir -p %s' % ADB_ROM_DIR)
    print(" *** 传输完成，正在解压缩，请稍后 ...")
    cmd('adb shell "busybox tar zxmof %s/\'%s\' -C %s"' % (TF_ROOT, ROM_PACK, ADB_ROM_DIR))
    cmd('adb shell "rm %s/\'%s\'"' % (TF_ROOT, ROM_PACK))

    print(" *** %s 输缩略图包大小 %d MB." % (MACHINE_NAME, int(os.path.getsize(DEST_THUMB_DIR+os.sep+THUMB_PACK)/1000/1000)))
    print(" *** 正在传输缩略图压缩包，请稍后 ...")
    cmd('adb push "%s" %s/' % (DEST_THUMB_DIR+os.sep+THUMB_PACK, TF_ROOT))
    cmd('adb shell "mkdir -p %s"' % ADB_THUMB_DIR)
    print(" *** 传输完成，正在解压缩，请稍后 ...")
    cmd('adb shell "busybox tar zxmof %s/\'%s\' -C %s"' % (TF_ROOT, THUMB_PACK, ADB_THUMB_DIR))
    cmd('adb shell "rm %s/\'%s\'"' % (TF_ROOT, THUMB_PACK))
    print()
