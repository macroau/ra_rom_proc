...
DEST_ROM_DIR=r'z:\GameEmu\天马\增量包\ra'
DEST_THUMB_DIR=r'z:\GameEmu\天马\增量包\th'
...
dirs=os.listdir(DEST_ROM_DIR)
for i in dirs:
    if os.path.isdir(DEST_ROM_DIR+os.sep+i):
        c='z: & cd "%s" & tar zcf "%s" "%s"' % (DEST_ROM_DIR, i+".tar.gz", i)
        print(c)
        r=os.system(c)
        print("errorlevel: ", r)
        
        c='z: & cd "%s" & tar zcf "%s" "%s"' % (DEST_THUMB_DIR, i+".tar.gz", i)
        print(c)
        r=os.system(c)
        print("errorlevel: ", r)
...
for i in os.listdir(DIR):
    ...
    for l in meta_fp.readlines():
        ...
        if game_name=="":
            if re.findall("^game", l):
                game_name=l.split(":")[1].strip()
                game_name= py.get_initials(game_name,'')[0] + " " + game_name
            ...
            if re.findall("^file", l):
                ...
                media_dir=DIR+os.sep+i+os.sep+"media" +os.sep    +file_name[:-4]
                
                box=media_dir+os.sep+"boxFront.jpg"
                #print(box)
                logo=media_dir+os.sep+"logo.jpg"

                if os.path.isfile(box):
                    print('.',end="")
                    sys.stdout.flush()
                    copyfile(box, type_th_dir+os.sep+'Named_Boxarts'+os.sep+game_name+box[-4:])
                        
                if os.path.isfile(logo):
                    print('.',end="")
                    sys.stdout.flush()
                    copyfile(logo, type_th_dir+os.sep+'Named_Titles'+os.sep+game_name+logo[-4:])            
